c-----------------------------------------------------------------------
c
c               ISARTLIB - General SAR Routines for Real-time
c
c      subroutine accel_rt ( z , n , b , dt , a )
c      subroutine dchirp_rt ( z , n , bavg , vel , sg0 , d0 , dd , dt ,
c      subroutine onelag_rt ( c , n , dt , nlag , dc , dw , d , pwr ,
c
C**
C***********************************************************************
C**
      subroutine accel_rt ( z , n , b , dt )
C**
C***********************************************************************
C**
      implicit none
...
C***********************************************************************
C**
      subroutine dchirp_rt ( z , n , bavg , vel , sg0 , d0 , dd , dt ,
...
c
      complex z(n) , cj
c
      real    b1 , b2 , b3 , bavg , dt , aavg , lambda , vel , d0 , dd ,
     .        sg0 , pi , arg , dc , dw , dcnew , dwnew , sg0new ,
     .        d0new , argnew , snr , snrold , snrnew , bavg0 , dtm2 ,
     .        rnd2
c
      logical done
c
...
c
      call onelag_rt ( z , n , dt , nlag , dc , dw , d0 , sg0 , arg )
c
      vel = dc * lambda / 2.0
      dd  = d0
c
...
c
         call accel_rt ( z(1)     , n / 2 , b1 , dt )
         call accel_rt ( z(1+n/4) , n / 2 , b2 , dt )
         call accel_rt ( z(1+n/2) , n / 2 , b3 , dt )
c
         aavg = 0.0                    ! ave amp variation
...
c
            call onelag_rt ( z , n / m2 , dtm2 , nlag , dcnew ,
...
c
               ...
               arg    = argnew
c
               call accel_rt ( z(1)        , n / m22 , b1 , dtm2 )
               call accel_rt ( z(1+n/m222) , n / m22 , b2 , dtm2 )
               call accel_rt ( z(1+n/m22)  , n / m22 , b3 , dtm2 )
c
               if ( nterms .eq. - 2 ) then
c
...
C***********************************************************************
C**
      subroutine onelag_rt ( c , n , dt , nlag , dc , dw , d , pwr ,
     .                       phi )
C**
C***********************************************************************
C**
...
c
         ...
         pwr  = pwr + c(i) * conjg( c(i) )  !  signal + noise
c
      enddo
c
...
c
      if ( real( pwr ) .eq. 0.0 ) then
c
         d = 1.0
c
      else
c
         d = amax1( 0.0 , 1.0 - cabs( cov ) / real( pwr ) )
c
      endif
c
