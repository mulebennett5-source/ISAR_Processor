...
C***********************************************************************
C**
      subroutine fourt1c ( rdata , nn , ndim , isign , iform , work ,
     .                     nwork )
C**
C***********************************************************************
C**
...
c
      real    rdata(2,nn(1)) , work(nwork)
c
      if ( ndim .eq. 1 .and. isign .gt. 0 ) then
c
         do i = 1 , nn(1) , 2
c
            rdata(1,i) = - rdata(1,i)
...
c
      call fourt ( rdata , nn , ndim , isign , iform , work , nwork )
c
      if ( ndim .eq. 1 .and. isign .lt. 0 ) then
c
         do i = 1 , nn(1) , 2
c
            rdata(1,i) = - rdata(1,i)
...
C***********************************************************************
C**
      subroutine fourt2c ( cdata , nn , ndim , isign , iform , work ,
     .                     nwork )
C**
C***********************************************************************
C**
...
c
      real    cdata(2,nn(1),nn(2)) , work(nwork)
c
      if ( ndim .eq. 2 .and. isign .gt. 0 ) then
c
         do j = 1 , nn(2)
c
            do i = 1 + mod(j,2) , nn(1) , 2
c
               cdata(1,i,j) = - cdata(1,i,j)
               cdata(2,i,j) = - cdata(2,i,j)
c
            enddo
c
         enddo
...
c
      call fourt ( cdata , nn , ndim , isign , iform , work , nwork )
c
      if ( ndim .eq. 2 .and. isign .lt. 0 ) then
c
         do j = 1 , nn(2)
c
            do i = 1 + mod(j,2) , nn(1) , 2
...
C**
      IMPLICIT NONE
C
      INTEGER NDIM , IDIM , ISIGN , IFORM , NTOT , N , NP1 , NP2 ,
     .        M , IF , IDIV , IQUOT , IREM , NTWO , J , ICASE , NON2 ,
     .        NP2HF , IMIN , IMAX , JMIN , JMAX , J1MAX , J2MAX , K ,
     .        NHALF , KMIN , KDIF , KSTEP , KRANG , KCONJ , J1 , J2 ,
     .        L , K3 , K4 , IFP1 , IFP2 , J1MIN , J2MIN , I2MAX , I ,
     .        J3MAX , I1 , I2 , I3 , J3 , I1MAX , NP0 , NPREV , NON2T ,
     .        IPAR , K1 , K2 , LMAX , I1RNG , MMAX , NP1HF ,
     .        J1RNG , J2STP , J1RG2 , J1CNJ , NWORK
C
      INTEGER NN(*) , IFACT(32)
C
      REAL WI , WR , WSTPR , WSTPI , TWOPI , SUMR , SUMI , OLDSR ,
     .     OLDSI , DIFR , DIFI , W2R , W2I , W3R , W3I , SINTH
      REAL U1R , U1I , U2R , U2I , U3R , U3I , U4R , U4I , T2R , T2I ,
     .     T3R , T3I , T4R , T4I , TEMPR , TEMPI , THETA , TWOWR
C**
      REAL DATA(*) , WORK(NWORK)
C**
      SAVE NP0 , NP1 , NPREV
C**
      WI    = 1.00
