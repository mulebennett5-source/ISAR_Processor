...
C**
     ...
     .                    wtac , wtaf , ascan , cframe , bytes , tgt ,
     .                    ctgt , pgatgt , dotdot , work , iwork ,
     .                    nwork , nabuff , mrrat , mprat , nchuse ,
     .                    mpass , isub , iframe , nafill_a , nfocus ,
     .                    tendat )
C**
C***********************************************************************
C**
...
c
     ...
     .             mrrat , mprat , nchuse , nafill_a , kframe
c
...
c
      integer     spulse , nwork , mpass , maxitu , nfocus , ntot , nti
c
      real        wtr(ntr) , wtac(3*naskip) , wtaf(nspf) ,
     .            ascan(nfr+nfa) , work(nwork) , pgatgt(2*nsr,5) ,
     .            dotdot(1+nfa/mprat,nfr/mrrat,nabuff) , vwt(ncr) ,
     .            tendat(npulse,11) , arbar , afbar , rmsdop , rmsacc ,
     .            ti_tot
c
...
c
     ...
     .            asmax , asbar , assdv , astemp , flight_path , vadd0 ,
     .            ground_path , image_path , mtrs2hz , vspot0 , dop ,
     .            vspot1 , vplat0 , vplat1 , omeg_0 , omeg_1 , asum ,
     .            asumlong , acor , tinteg_1
c
     ...
     .            nspf_use , itemp , rt_frames , nn(1) , nxmid , sip ,
     .            nxtotal , gpulse , gpulse0 , gpulse1 , k_vadd , n_asum
c
      complex     cpfast
c
...
c
      integer     icount , nptgt , sframe
c
      real        dtpga , db , corg , cnew , c_reduce , ctest ,
     .            ctestmax , vaddmax
c
...
c
c   In Ultra Wide Band mode the sub-image range cell size is
...
c
c   In RDRTec real-time mode fill the array with a cosine-on-a-pedestal
c   weight function
c
...
c
      sframe = 0                       ! Counter for covariance data
c
...
c
      eslip  = 0.0                     !  Corr. for incorrect range walk
c
...
c
                  sip = ipulse+1-spulse
                  dr0 = tendat(sip,1)+rcentr
                  if ( sip .lt. npulse ) then
                     vr0 = (tendat(sip+1,1)-tendat(sip,1))/dtp
                  endif
c
               else
c
...
c
c   Correct for 'slip' of the mo-comp point (normally for NRL Advanced
c   Profile Radar only)
c
                  if ( alias .eq. 2 ) then
c
                     vslip = vslip + 0.5 * eslip / float( 2 * naskip )
c
                  endif
c
...
c
                     read  ( 87 , '(4f16.6)' )
     .                       tdummy , dr0 , vr0 , vslip
c
                  else
c
                     write ( 87 , '(5f16.6)' )
     .                       float( ipulse - spulse ) * dtp , dr0 ,
     .                       vr0 , vslip , eslip
c
                  endif
c
...
c
               if ( finemc .ne. 0 ) then
c
                  if ( abs(finemc) .eq. 1 .or. abs(finemc) .eq. 5 ) then
c
                     nr1fm = nr1
c
                     nrefm = nre
...
c
                  if ( isub .gt. 1 .and. finemc .lt. 0 ) then
c
c                   Implement adaptive amplitude correction
c                 
                     asum         = 0.0
                     do ir = nr1fm , nrefm
c
                        asum = asum + vwt(ir) * abs(cstr(ip,ir,1)) ** 2
c
                     enddo
c
                     if ( n_asum .le. 400 ) then
                        asumlong = asumlong + asum
                        n_asum   = n_asum + 1
                     else
                        asumlong = 0.9975 * asumlong + asum
                     endif
c
                     acor         = 1.0 / sqrt(asum/(asumlong/n_asum))
c
                     cstr(ip,:,:) = cstr(ip,:,:) * acor
c
                  endif
c
               endif
...
c
            if ( ( mode .ne. 10 ) .and.
     .         ( ( isub .le. 2 .or. mod( isub , 2 ) .eq. 0 ) ) ) then
c
...
c
            call subimg ( mpass , isub , csbimg(1,1,iisub) , nsa , nsr ,
...
c
               if ( nfskip .lt. 0 ) then  ! Variable integration time
c
c   Compute a recommended image integration time from the target
c   Doppler statistics
c
                  call choose_tinteg( tinteg_1 , ti_con , tframe , dts ,
     .                                nspf/2 , rmsdop , rmsacc , ntot )
c
                  if ( tinteg_1 .gt. 0 ) then
                  
                     if ( ti_con .gt. 0 ) then
c
c   Average the recommended integration time over all previous times
c
                        nti        = nti+1
                        ti_tot     = ti_tot+tinteg_1
                        tinteg_use = ti_tot/nti
c
                     else
c
c   Use the present value of the recommended integration time
c
                        tinteg_use = tinteg_1
c
                     endif
...
c
c   No valid value of recommended integration time - just use the
c   standard value
c
                        tinteg_use = tinteg
c
...
c
               if ( mode .eq. 10 ) then
c
                  call getAccTen ( 1 + nfa / mprat , nfr / mrrat , 
     .                             nspfv , dotdot ,
     .                             fcenuse - 0.5 * float( nfa ) * dff ,
     .                             rcenuse + rfmin + 0.5 * drf * mrrat ,
     .                             tv0 , dfmp , drfbig , dtv , lambda ,
     .                             tendat(1,5) , tendat(1,2) ,
     .                             tendat(1,3) , tendat(1,4) , rcen ,
     .                             fcen , npulse , arbar , afbar )
c
                  call gettgt_s ( csbimg , nakeep , nsr , nabuff ,
     .                            pnoise , pgatgt , iwork , dts ,
     .                            iisub , isub , 0.0 , work , nptgt ,
     .                            work , curtime , arbar , afbar ,
     .                            sframe )
c
               elseif ( rt_img .eq. 0 .or. color .ne. 0 ) then
c
...
c
                  vadd0    = vadd
c
                  do k_vadd = - n_vadd , n_vadd
c
                  vadd = vadd0 + d_vadd * float( k_vadd )
c
     ...
     .                            reduce )
c
                  ctest = ctrast( cframe , nfa , nfr )
c
                  if ( ctest .gt. ctestmax ) then
c
                     ctestmax = ctest
c
                     vaddmax  = vadd
...
c
                  cframe(:,:,1) = cframe(:,:,2)
c
                  if ( d_vadd .gt. 0.0 ) then
c
c                    call vaddcor ( cframe , cframe(1,1,2) , nfa , nfr ,
c    .                              vaddmax , dff , drf )
c
c                    vadd = vadd + vaddmax
c
...
c
               call reduce_streaks ( cframe , nfa , nfr , 1.0 , 0.5 ,
     .                               60.0 )
c
...
c           
            write ( 82 , '(1x,10f12.4,2i8)' )  tframe , db( corg ) ,
     .                                    db( cnew ) , db( c_reduce ) ,
     .                                    vadd , tinteg_use , tint ,
     .                                    rmsdop , rmsacc , dtv , ntot ,
     .                                    nspf_use
c
            if ( quiet .gt. 1 ) write( 6 , '(1x,a26,10f12.4,2i8)' )
     .                          ' Autofocus efficiency:    ' , tframe ,
     .                            db( corg ) , db( cnew ) ,
     .                            db( c_reduce ) , vadd , tinteg_use ,
     .                            tint , rmsdop , rmsacc , dtv , ntot ,
     .                            nspf_use
c
c-----------------  Phase Gradient Autofocus ---------------------------
c-----------------------------------------------------------------------
...
c
     ...
     .                          rlook , flook )
c
            else if ( mpass .eq. 0 .or. mpass .eq. 2 ) then
c
               if ( otype .eq. 0 ) then
c
                  pframe = + iframe
